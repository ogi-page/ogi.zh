...
function clearHighlights() {
  document.querySelectorAll('#dynamic-board .square').forEach(square => {
...
async function initializeGame() {
  ...
  if (moveAbilitiesData) {
      const squares = document.querySelectorAll('#dynamic-board .square');

      squares.forEach(square => {
          square.addEventListener('mouseenter', function() {
              const piece = this.querySelector('#dynamic-board .piece');
              if (piece) {
